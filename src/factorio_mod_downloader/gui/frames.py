...
class DownloaderFrame(customtkinter.CTkScrollableFrame):
    ...
    def _setup_downloads_frame(self, label: str):
        ...
        downloads_frame.pack(fill="x", pady=5)
...
class BodyFrame(customtkinter.CTkFrame):
    ...
    def _setup_body_frame(self):
        ...
        self.body_frame.rowconfigure(4, weight=1)  # Dostosowanie do dodatkowego wiersza
        ...
        # --- NOWA SEKCJA: WYBÓR WERSJI FACTORIO ---
        version_container = customtkinter.CTkFrame(master=self.body_frame, fg_color="transparent")
        version_container.grid(row=2, column=0, columnspan=4, padx=10, pady=(0, 10), sticky="nsew")
        
        version_label = customtkinter.CTkLabel(
            master=version_container,
            text="Factorio Version:",
            font=customtkinter.CTkFont(family="Segoe UI", weight="bold")
        )
        version_label.pack(side="left", padx=(0, 10))

        # Lista wersji - domyślnie 2.0, dodane też 2.1 oraz starsza 1.1 dla kompatybilności
        self.factorio_version = customtkinter.CTkComboBox(
            master=version_container,
            values=["2.0", "2.1", "1.1"],
            width=100
        )
        self.factorio_version.set("2.0")  # Domyślnie ustawiona wersja 2.0
        self.factorio_version.pack(side="left")
        # ------------------------------------------
        ...
        self.download_button.grid(
            row=4, column=0, columnspan=4, padx=10, pady=(0, 10), sticky="nsew"
        )
    ...
    def _validate_inputs(self) -> bool:
        ...
        mod_url_raw = self.mod_url.get().strip()
        ...
        if not mod_url_raw:
            CTkMessagebox(
                title="Error",
                width=500,
                message="Please provide at least one mod URL!!!",
                icon="cancel",
            )
            return False

        # Dzielimy po przecinku i czyścimy białe znaki z każdego linku
        urls = [u.strip() for u in mod_url_raw.split(",") if u.strip()]
        
        # Sprawdzamy, czy każdy z podanych linków jest poprawnym adresem modu Factorio
        for url in urls:
            if not re.match(r"^https://mods\.factorio\.com/mod/.*", url):
                CTkMessagebox(
                    title="Error",
                    width=500,
                    wraplength=500,
                    message=f"Invalid mod URL detected:\n{url}\nPlease ensure all URLs are separated by commas.",
                    icon="cancel",
                )
                return False
    ...
    def _download_button_action(self):
        ...
        try:
            ...
            # Tworzymy listę wyczyszczonych linków i przekazujemy ją do ModDownloadera
            mod_urls = [u.strip() for u in mod_url_raw.split(",") if u.strip()]
